...
class PointsDeal(object):
	...
	def points_zero_degree_deal(self, points_degree_zero, pattern="front"):
		"""
		...
		if pattern=="front":
			...
			return [100, 100, 100, 100, right_mean, right_std, 100, 100]
			...
			indices_left = np.where((points_zero_degree_x < 0) & (-0.5 < points_zero_degree_y) & (points_zero_degree_y <0.5))
			indices_right = np.where((points_zero_degree_x > 0) & (-0.5 < points_zero_degree_y) & (points_zero_degree_y <0.5))
			...
			return [front_mean, front_std, left_mean, left_std, right_mean, right_std ,100, 100]
