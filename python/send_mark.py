...
def send_mark():
    ...
    mark_pub = rospy.Publisher('/path_point', MarkerArray, queue_size = 100)  # rviz发布目标点
    click_sub = rospy.Subscriber('/clicked_point', PointStamped, press_callback)  # 接受rviz的目标点

    goal_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size = 1)  # 发布机器人的目标点
    goal_status_sub = rospy.Subscriber('/move_base/result', MoveBaseActionResult, pose_callback)  # 接受机器人的最终位置
    goal_status_pub = rospy.Publisher('/move_base/result', MoveBaseActionResult, queue_size = 1)  # 发布机器人的最终位置
    rospy.spin()
