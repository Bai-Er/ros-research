...
def Center_detect(cmd_vel_pub, road_width=1.1):
    """
    校正车辆行驶的方向用
...
def turn_left(cmd_vel_pub, speed, rotate_assert=None):
    ...
    if rotate_assert is not None:
        ...
        print("test")
        go_straight = int (math.pi / (2 * speed) * send_rate + 10) # 后面的2自己修改, 用于校正摩擦力
        print(go_straight)
        while go_straight:
            cmd_vel_pub.publish(cmd_vel_msg)
            rate.sleep()
...
def main(cmd_vel_pub):
    ...
    while True:
	    ...
	    # 直走直到前方距离是0.5m, 里面有个死循环，没有执行完的时候是不会跳出这个函数的
    	    Center_detect(cmd_vel_pub, road_width = 1.2)
            front_go -= 1
            print(front_go)
    ...
    go_straight_x_until(cmd_vel_pub, speed=0.1, distance=1.2)
    turn_left(cmd_vel_pub, speed=0.5) # 左转90度
    go_straight_x(cmd_vel_pub, 0.7, distance=10)
    turn_left(cmd_vel_pub, speed=0.5) # 左转90度
    go_straight_x(cmd_vel_pub, 0.7, distance=2)
    ...
    turn_right(cmd_vel_pub, speed=-0.5, add=14) # 左转90度
    go_straight_x(cmd_vel_pub, 0.7, distance=2.5)
    turn_right(cmd_vel_pub, speed=-0.5, add=12) # 左转90度
    go_straight_x(cmd_vel_pub, 0.7, distance=3)
    print("----end circle----")
